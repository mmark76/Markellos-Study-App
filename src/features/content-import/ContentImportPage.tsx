...
async function readTemplateFile(file: File): Promise<unknown> {
...
export function ContentImportPage() {
  ...
  async function importUnits(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const units = parseImportedUnits(await readTemplateFile(file));
      ...
      setMessage("The units file does not match the required template.");
    } finally {
      event.target.value = "";
    }
  }
  ...
  async function importFlashcards(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const flashcards = parseImportedFlashcards(await readTemplateFile(file));
      ...
      setMessage("The flashcards file does not match the required template.");
    } finally {
      event.target.value = "";
    }
  }
  ...
  return (
    <div className="stack-lg">
      <header className="page-heading">
        ...
        <p>Download a template, replace the example content with your own, and import the completed file.</p>
      </header>
      ...
      <section className="content-panel">
        ...
        <div className="button-row">
          <a
            ...
            download="units-template.json"
            href={`${import.meta.env.BASE_URL}templates/units-template.json`}
          >
            Download units template
          </a>
          <a
            className="button secondary"
            download="flashcards-template.json"
            href={`${import.meta.env.BASE_URL}templates/flashcards-template.json`}
          >
            Download flashcards template
          </a>
        </div>
      </section>

      <section className="content-panel">
        <h3>Import completed files</h3>
        <div className="button-row">
